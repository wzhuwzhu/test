...
class StaffEdit2 extends Component {
  ...
  validateField(fieldName, value) {
      ...
      switch(fieldName){
      case 'name':
    	  nameValid = (value==null||(value=value.trim()).length==0)? false :
			  (isNaN(value));
    	  fieldValidationErrors.name = nameValid ? '' : ' is invalid';
          break;
      case 'yearOfGraduation':
    	  yearOfGraduationValid = (value==""||value==null||value.trim().length==0)?
    			  true:(/^(19|20)\d{2}$/.test(value));
    	  fieldValidationErrors.yearOfGraduation = yearOfGraduationValid ? '' : ' is invalid';
          break;    
      case 'enrolledSince':
    	  enrolledSinceValid = (value==""||value==null||value.trim().length==0)?
    			  true:(/^(19|20)\d{6}$/.test(value));
    	  fieldValidationErrors.enrolledSince = enrolledSinceValid ? '' : ' is invalid';
          break; 
      case 'phoneNo':
    	  phoneNoValid = (value==""||value==null||value.trim().length==0)?
    			  true:(/^(1)\d{10}$/.test(value));
    	  fieldValidationErrors.phoneNo = phoneNoValid ? '' : ' is invalid';
          break; 
      case 'yearOfBirth':
    	  yearOfBirthValid = (value==""||value==null||value.trim().length==0)?
    			  true:(/^(19|20)\d{2}$/.test(value));
  ...
  this.setState({formErrors: fieldValidationErrors,
	  nameValid: nameValid,
      yearOfGraduationValid: yearOfGraduationValid,
      enrolledSinceValid: enrolledSinceValid,
        ...
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormGroup>
            <Button 
              onClick={(event)=>{
            	  return(
            		<EditModal staff = {this.state.staff}	  
            	  )
            	  this.validateField('name',staff.name);
            	  this.validateField('yearOfGraduation',staff.yearOfGraduation);
            	  this.validateField('enrolledSince',staff.enrolledSince);
            	  this.validateField('phoneNo',staff.phoneNo);
            	  this.validateField('yearOfBirth',staff.yearOfBirth);
            	  this.validateForm();
            	  console.log({nameValid},{yearOfGraduationValid},
            			  {enrolledSinceValid},{phoneNoValid},
            			  {yearOfBirthValid},{formValid});
            	  } 
              }
